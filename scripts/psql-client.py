...
def configuration(defaults=None):
    if defaults is None:
        defaults = {
            "config": os.path.expanduser("~/.pg_service.conf"),
            "section": "postgres",
        }
    # create a parser
    parser = ConfigParser()
    ...
    if parser.has_section(defaults["section"]):
        ...
        raise Exception(
            "Section {} not found in the {} file".format(
                defaults["section"], defaults["config"]
            )
        )
    return defaults
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Connect to PostgreSQL and run basic queries."
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Section name in config file (overrides PGSESSION)",
    )
    try:
        ...
        print(error)
    parser.add_argument(
        "--host", type=str, default=config["host"], help="Host(fqdn or ip) to connect"
    )
    parser.add_argument(
        "--port", type=int, default=config["port"], help="Port(number) to connect"
    )
    parser.add_argument(
        "--database",
        type=str,
    ...
    parser.add_argument(
        "--user", type=str, default=config["user"], help="User/role(name) to connect"
    )
    parser.add_argument(
        "--password",
        type=str,
        default=config["password"],
        help="User/role(credentials) to connect",
    )
    parser.add_argument(
        "--sslmode",
        type=str,
        default=config["sslmode"],
        help="SSL Mode - disable, require, verify, verify-full",
    )
    parser.add_argument(
        "--sslcert",
        type=str,
        default=config["sslcert"],
        help="SSL Certificate(file path) -- Certificate authorization certificate",
    )
    parser.add_argument(
        "--sslkey",
        type=str,
        default=config["sslkey"],
        help="SSL Private Key(file path) -- Certificate authorization private key",
    )
    parser.add_argument(
        "--sslrootcert",
        type=str,
        default=config["sslrootcert"],
        help="SSL Public Certificates(file path) -- OS certificate list",
    )
